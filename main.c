...
int main(int argc, char *argv[]) {
    ...
    if (argc != 4) {
        printf("Usage: ./a.out searchMode keyFileName seekFileName\n");
        exit(1);
    }
    ...
    // Initialize hit for all numbers to 0
    for (i = 0; i < seeksLength; ++i) {
        hits[i] = 0;
    }

    gettimeofday(&startTime, NULL);
    // Keep all keys in memory and do a linear search
    ...
    // Read each key from disk and do a linear search
    ...
    // Keep all keys in memory and do a binary search
    ...
    // Read each key from disk and do a binary search
    ...
    // Print Yes or No based on hit status
    for (i = 0; i < seeksLength; ++i) {
        printf("%12d: ", seeks[i]);
        if (hits[i] == 1) {
            printf("Yes\n");
